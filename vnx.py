...
class EMCVnxBlockDeviceAPI(object):
    ...
    def _get_lun_name_from_blockdevice_id(self, blockdevice_id):
        """
        """
        return LUN_NAME_PREFIX + cluster_id + str(blockdevice_id)

    def _get_blockdevice_id_from_lun_name(self, lun_name):
        """
        """
        return unicode(lun_name.split(LUN_NAME_PREFIX + cluster_id, 1)[1])
    ...
    def list_volumes(self):
        ...
        for each in luns:
            if each['lun_name'].startswith(LUN_NAME_PREFIX + cluster_id):
                attached_to = None
